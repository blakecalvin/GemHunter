...
public class Screen {
    ...
    public void render(int xPos, int yPos, int tile){
        render(xPos, yPos, tile, 0x00, BLACK , 1);
    }

    public void render(int xPos, int yPos, int tile, int colour){
        render(xPos, yPos, tile, 0x00, colour, 1);
    }

    public void render(int xPos, int yPos, int tile, int mirrorDir, int colour, int scale){
        xPos -= xOffset;
        yPos -= yOffset;
        ...
        for(int y=0; y<8; y++){
            ...
            for(int x=0; x<8; x++){
                ...
                if(sheet.pixels[xSheet+ySheet*sheet.width+tileOffset] == -1){
                    pixels[(x + xPos) + (y + yPos) * width] = colour;
                }
                else if(sheet.pixels[xSheet+ySheet*sheet.width+tileOffset]!= BLACK) {
